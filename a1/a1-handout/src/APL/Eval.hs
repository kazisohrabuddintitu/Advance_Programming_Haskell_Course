...
-- New case for lamda function
eval env (Lambda vname exp) = Right $ ValFun env vname exp

-- New case for Apply function
eval env (Apply exp1 exp2) = 
  case eval env exp1 of 
    Left err -> Left err
    Right (ValFun environment parameter body) ->
      case eval env exp2 of
        Left err -> Left err
        Right expVal ->
          eval (envExtend parameter expVal environment) body
    (Right _ ) -> Left "non functional value"      


-- New case for TryCatch function
eval env (TryCatch exp1 exp2) = 
  case eval env exp1 of
    Right result -> Right result
    Left _ -> case eval env exp2 of
                Right result -> Right result
                Left err2 -> Left err2
